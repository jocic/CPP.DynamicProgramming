...
TEST_CASE("[004-howSum] DP implementation - unoptimized...", "[004-howSum]") {
    ...
    CHECK(how_sum_dp(100, { 7, 14, 2 }) == vector<uint64_t>{ 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 2 });
}
...
TEST_CASE("[004-howSum] DP implementation - Memoization pattern...", "[004-howSum]") {
    ...
    CHECK(how_sum_dp_memo(100, { 7, 14, 2 }) == vector<uint64_t>{ 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 2 });
}
...
TEST_CASE("[004-howSum] DP implementation - Tabulation pattern...", "[004-howSum]") {
    ...
    CHECK(how_sum_dp_tab(7, { 3, 2 }) == vector<uint64_t>{ 3, 2, 2 });
    ...
    CHECK(how_sum_dp_tab(100, { 7, 14, 2 }) == vector<uint64_t>{
...
TEST_CASE("[004-howSum] Benchmarks...") {

    BENCHMARK("DP - Unoptimized") {
        return how_sum_dp(100, { 7, 14, 3, 2 });
    };

    BENCHMARK("DP - Memoization") {
        return how_sum_dp_memo(100, { 7, 14, 3, 2 });
    };

    BENCHMARK("DP - Tabulation") {
        return how_sum_dp_tab(100, { 7, 14, 3, 2 });
    };
}
