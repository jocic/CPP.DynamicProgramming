...
TEST_CASE("[004-howSum] DP implementation - Tabulation pattern...", "[004-howSum]") {
    ...
    CHECK(how_sum_dp_tab(7, { 2, 3 }) == vector<uint64_t>{ 2, 2, 3 });
    CHECK(how_sum_dp_tab(7, { 3, 2 }) == vector<uint64_t>{ 2, 2, 3 });
    CHECK(how_sum_dp_tab(7, { 5, 3, 4, 7 }) == vector<uint64_t>{ 7 });
    CHECK(how_sum_dp_tab(8, { 2, 3, 5 }) == vector<uint64_t>{ 3, 5 });
...
TEST_CASE("[004-howSum] Benchmarks...") {
    ...
    BENCHMARK("DP - Unoptimized") {
        return how_sum_dp(100, { 1, 1, 1 });
    };

    BENCHMARK("DP - Memoization") {
        return how_sum_dp_memo(100, { 1, 1, 1 });
    };

    BENCHMARK("DP - Tabulation") {
        return how_sum_dp_tab(100, { 1, 1, 1 });
    };
}
